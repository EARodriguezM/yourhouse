...
const Home = (): JSX.Element => {
  ...
  if (error) return <ErrorMessage message="Error al obtener propiedades" />;
  ...
  const reachedEndElement = isReachedListingsEnd ? (
    <ReachedEndText> Has llegado al final. </ReachedEndText>
  ) : null;

  const viewMoreElement = !isReachedListingsEnd && !isLoadMore && (
    <ViewMoreButton title="Ver más" onClick={loadMore} />
  );
  ...
  return (
    <>
      ...
      <Container>
        <FeaturedCities />
        <Title> Propiedades que te pueden gustar </Title>
        {loading ? (
