...
const CreateListing = (): JSX.Element => {
  ...
  const [createListing, { loading }] = useMutation(CREATE_LISTING, {
    onError() {
      setToast('error', 'Error en añadir la propiedad. Por favor, intente mas tarde');
    },
    onCompleted(data) {
      history.push(`/listing/${data.createListing.id}`);
      setToast('success', 'Propiedad añadida exitosamente');
    },
  });
  ...
  const buttonSubmitText = isLastStep ? 'Añadir propiedad' : 'Siguiente';
