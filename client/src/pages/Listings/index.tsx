...
const ListingsPage = (): JSX.Element => {
  ...
  const renderListings = () => {
    ...
    if (error) {
      return (
        <ErrorMessage message="Error al buscar propiedades. Por favor, inténtelo de nuevo más tarde." />
      );
    }
    if (listings.total === 0) {
      return (
        <EmptyMessage
          ...
          description="Intente buscar otra ubicación."
        />
      );
    }
  ...
  return (
    <Container>
      ...
      <SearchText>
        Busqueda de propiedades en <span>{location}</span>
      </SearchText>
      <Wrapper>
        <ListingsFilterContainer>
          <ListingsFilter />
